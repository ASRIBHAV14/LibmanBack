...
@Service
public class BooksServiceImpl implements BooksServ{
@Autowired
...
public Books addBook(BooksDto book) {
	...
	Optional<Publishers> publisher = publisherrepo.findBypublisherName(book.getPublisherName());
	Publishers p1 = publisher.get();
	r1.setPublisher(p1);
	Optional<Author> author = authorrepo.findByFirstName(book.getFirstName());
...
public BooksDto searchBookByTitle(String title) throws Throwable {
	...
	dto.setFirstName(std.getAuthor().getFirstName());
...
public BooksDto searchBookBySubject(String subject) throws Throwable {
	...
	dto.setPublishedyear(std.getPublishedyear());
	dto.setFirstName(std.getAuthor().getFirstName());
	dto.setPublisherName(std.getPublisher().getPublisherName());
	return dto;
}
